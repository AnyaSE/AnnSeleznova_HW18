let users = [
    ...
    const filteredUsers = users.filter(balanceCheck);

    function onlyPhone (users) {
        return users.phone;
    }

    const onlyPhoneArray = filteredUsers.map(onlyPhone);
    ...
    const sumOfAllBalances = users.reduce(addAllBalances, 0);
